...
import { FiEdit2, FiTrash2, FiCheck, FiX, FiRadio, FiCpu, FiCopy } from 'react-icons/fi';
...
export default function RadioManager({ radioManager }: RadioManagerProps) {
  ...
  const handleCopyId = useCallback((id: string) => {
    copy(id);
    toast.success(i18n.__('ID copied'));
  }, []);

  const handleCopyAlias = useCallback((alias: string) => {
    copy(alias);
    toast.success(i18n.__('Name copied'));
  }, []);

  return (
    <div className="bg-white border border-gray-200 shadow-sm mt-4">
      {/* Header */}
      <div className="px-4 py-3 bg-gray-50 border-b border-gray-200 flex items-center justify-between gap-6">
        <div className="flex items-center gap-3">
          <h2 className="font-semibold text-gray-800">{i18n.__('Paired Devices')}</h2>
          <span className="px-2 py-0.5 text-xs font-medium bg-gray-200 text-gray-700 rounded-full">
            {nodes.length} {nodes.length === 1 ? i18n.__('device') : i18n.__('devices')}
          </span>
        </div>
        ...
        >
          {pairingMode ? (
            <>
              ...
              {i18n.__('Stop Pairing')}
            </>
          ) : (
            <>
              <FiRadio className="w-3.5 h-3.5" />
              {i18n.__('Start Pairing')}
            </>
          )}
        </button>
      </div>
                  ...
                  <div className="flex items-center justify-between gap-4">
                      ...
                      <div className="min-w-0 flex-1">
                        {isEditing ? (
                          <input
                            ...
                            placeholder={i18n.__('Enter device alias')}
                          />
                        ) : (
                          <>
                            <div className="flex items-center gap-1">
                              <p className="font-medium text-gray-900 truncate">
                                {item.alias || <span className="text-gray-400 italic">{i18n.__('No alias')}</span>}
                              </p>
                              {item.alias && (
                                <button
                                  onClick={() => handleCopyAlias(item.alias!)}
                                  className="p-1 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded transition-colors"
                                  title={i18n.__('Copy name')}
                                >
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      {isEditing ? (
                        ...
                        <>
                          <span className="text-sm text-red-600 mr-2">{i18n.__('Delete?')}</span>
                          <button
                            onClick={() => handleRemove(item)}
                            className="p-2 text-red-600 hover:bg-red-50 rounded transition-colors"
                            title={i18n.__('Confirm delete')}
                          >
                            <FiCheck className="w-4 h-4" />
                          </button>
                          <button
