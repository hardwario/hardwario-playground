...
export default class extends Component {
    ...
    ipcGatewayStatus(sender, status) {
        this.setState({ gatewayOnline: status == "online" });
    }

    ipcGatewayDevice(sender, devide) {
        this.setState({selectedPort: devide});
    }
    ...
    render() {
        return (
            <div className="gatewayPortList form-inline">
                ...
                <Button disabled={this.state.ports.length == 0} color={this.state.gatewayOnline ? "danger": "success"} onClick={this.buttonOnClick}>{this.state.gatewayOnline ? i18n.__("Disconnect") : i18n.__("Connect")}</Button>
            </div>
        )
    }
}
