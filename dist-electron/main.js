...
var escapeGoat = {};
(function(exports$1) {
  exports$1.htmlEscape = (string) => string.replace(/&/g, "&amp;").replace(/"/g, "&quot;").replace(/'/g, "&#39;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  exports$1.htmlUnescape = (htmlString) => htmlString.replace(/&gt;/g, ">").replace(/&lt;/g, "<").replace(/&#0?39;/g, "'").replace(/&quot;/g, '"').replace(/&amp;/g, "&");
  exports$1.htmlEscapeTag = (strings, ...values) => {
    let output = strings[0];
    for (let i = 0; i < values.length; i++) {
      output = output + exports$1.htmlEscape(String(values[i])) + strings[i + 1];
    }
    return output;
  };
  exports$1.htmlUnescapeTag = (strings, ...values) => {
    let output = strings[0];
    for (let i = 0; i < values.length; i++) {
      output = output + exports$1.htmlUnescape(String(values[i])) + strings[i + 1];
    }
    return output;
  };
})(escapeGoat);
...
const path = path$2;
const { app, BrowserWindow, shell, dialog } = require$$0$1;
...
const create = (win, options) => {
  const handleContextMenu = (event, props) => {
    ...
    const shouldShowInspectElement = typeof options.showInspectElement === "boolean" ? options.showInspectElement : isDev$1;
...
const NodeREDWorker = require$1("../src/background/NodeREDWorker");
const BlocklyWorker = require$1("../src/background/BlocklyWorker");
const MqttBroker = require$1("../src/background/MqttBroker");
const Settings = require$1("../src/background/Settings");
const Firmware = require$1("../src/background/Firmware");
const Gateway = require$1("../src/background/Gateway");
const Home = require$1("../src/background/Home");
const CustomMenu = require$1("../src/utils/Menu");
contextMenu({});
const windows = [];
const isDev = process.env.NODE_ENV === "development" || process.defaultApp || /[\\/]electron-prebuilt[\\/]/.test(process.execPath) || /[\\/]electron[\\/]/.test(process.execPath);
const VITE_DEV_SERVER_URL = process.env.VITE_DEV_SERVER_URL || "http://localhost:5173";
function createWindow() {
  if (!require$$0$1.app.isReady()) return;
  const mainWindow = new require$$0$1.BrowserWindow({
    width: 1e3,
    minWidth: 1e3,
    minHeight: 480,
    show: false,
    icon: path$2.join(__dirname$1, "..", "src", "assets", "icons", "icon.png"),
    webPreferences: {
  ...
  mainWindow.on("close", (e) => {
    ...
    closeTimer = setTimeout(() => {
      require$$0$1.dialog.showMessageBox(mainWindow, {
        type: "warning",
...
require$$0$1.ipcMain.handle("app/getPath", (_event, name) => require$$0$1.app.getPath(name));
require$$0$1.ipcMain.on("shell:openExternal", (_event, url2) => {
  try {
    ...
    console.error("Invalid URL for openExternal:", url2);
  }
});
...
require$$0$1.app.on("ready", createWindow);
require$$0$1.app.on("activate", () => {
  if (windows.length === 0) {
    createWindow();
  }
});
require$$0$1.app.on("app:window:new", createWindow);
